    ...
    if (submitButton) {
        submitButton.addEventListener('click', () => {
            ...
            if (selectedOption) {
                ...
                if (!correctAnswers.includes(4) && !incorrectAnswers.includes(4)) {
                    if (answer === 'correct') {
                        score += 10;
                        correctAnswers.push(4); // Record question 2 as correct
                        localStorage.setItem('score', score);
                        localStorage.setItem('correctAnswers', JSON.stringify(correctAnswers));
                        window.location.href = 'correct_page_q4.html';
                    } else {
                        incorrectAnswers.push(4); // Record question 2 as incorrect
