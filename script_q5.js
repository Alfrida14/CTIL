    ...
    if (submitButton) {
        submitButton.addEventListener('click', () => {
            ...
            if (selectedOption) {
                ...
                if (!correctAnswers.includes(5) && !incorrectAnswers.includes(5)) {
                    if (answer === 'correct') {
                        score += 10;
                        correctAnswers.push(5); // Record question 2 as correct
                        localStorage.setItem('score', score);
                        localStorage.setItem('correctAnswers', JSON.stringify(correctAnswers));
                        window.location.href = 'correct_page_q5.html';
                    } else {
                        incorrectAnswers.push(5); // Record question 2 as incorrect
